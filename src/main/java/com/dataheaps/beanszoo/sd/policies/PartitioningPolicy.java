...
public class PartitioningPolicy implements Policy {
    ...
    public Object getServiceInstance(Class klass, String name, List<ServiceDescriptor> d, RpcClient client, ServiceDirectory services) {
        ...
        return Proxy.newProxyInstance(this.getClass().getClassLoader(), new Class<?>[]{klass}, new InvocationHandler() {
            ...
            Object invokeMultipleInstances(Partitioner p, List<ServiceDescriptor> d, Method m, List<Object[]> args) throws Exception {
                ...
                for (int ctr=0;ctr<d.size();ctr++) {
                    ...
                    executors.submit(() -> {
                       try {
                           Object res = invokeSingleInstance(d.get(index), m, args.get(index));
            ...
            Object invokeSingleInstance(ServiceDescriptor d, Method m, Object[] args) throws Exception {
            ...
            public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
                ...
                if (partitioned == null) {
                    return invokeSingleInstance(sdl.get(Math.abs(rand.nextInt()) % sdl.size()), method, args);
                }
                else {

                    Partitioner p = (Partitioner) partitioned.partitioner().newInstance();

                    int partitionKeyIndex = getPartitionKeyIndex(method);
                    if (partitionKeyIndex < 0) {
                        return invokeMultipleInstances(
                                p, sdl, method, sdl.stream().map(e -> args).collect(Collectors.toList())
                        );
                    }
                    else {
